...
namespace {
    ...
    define('LARAVEL_URL', 'http://www.obdjuv.org');
    define('BRIDGEBB_API_KEY', 'youthobservatory'); // Api key

    // User properties from laravel as key and phpBB as value
    define ('LARAVEL_CUSTOM_USER_DATA', serialize ([
        'email' => 'user_email',
        'nascimento' => 'user_birthday',
    ]));
...
namespace laravel\bridgebb\auth\provider {
    class bridgebb extends \phpbb\auth\provider\base
    {
        ...
        public function login($username, $password)
        {
		 ...
		 && ($this->_getUserByUsername($username) || strpos($_SERVER['REQUEST_URI'], "/forum/adm/"))) {
